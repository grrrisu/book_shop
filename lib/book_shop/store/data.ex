defmodule BookShop.Store.Data do
  def data do
    [
      %{
        name: "Utopia",
        price: 1500,
        author: "Thomas More",
        category: "Philosophical Fiction",
        pages: 200,
        isbn: "9798682441006",
        image_url: "https://prodimage.images-bn.com/pimages/9798682441006_p0_v1_s1200x630.jpg"
      },
      %{
        name: "Alice in Wonderland",
        price: 1300,
        author: "Lewis Carroll",
        category: "Children's Fantasy",
        pages: 192,
        isbn: "9780486275437",
        image_url: "https://covers.openlibrary.org/b/isbn/9780486275437-L.jpg"
      },
      %{
        name: "Dune",
        price: 1700,
        author: "Frank Herbert",
        category: "Science Fiction",
        pages: 688,
        isbn: "9780441172719",
        image_url: "https://covers.openlibrary.org/b/isbn/9780441172719-L.jpg"
      },
      %{
        name: "The Hitchhiker's Guide to the Galaxy",
        price: 1400,
        author: "Douglas Adams",
        category: "Science Fiction Comedy",
        pages: 224,
        isbn: "9780345391803",
        image_url: "https://covers.openlibrary.org/b/isbn/9780345391803-L.jpg"
      },
      %{
        name: "Neuromancer",
        price: 1600,
        author: "William Gibson",
        category: "Cyberpunk",
        pages: 271,
        isbn: "9780441569595",
        image_url: "https://covers.openlibrary.org/b/isbn/9780441569595-L.jpg"
      },
      %{
        name: "Elixir in Action",
        price: 4500,
        author: "Saša Jurić",
        category: "Programming",
        pages: 384,
        isbn: "9781617295027",
        image_url: "https://covers.openlibrary.org/b/isbn/9781617295027-L.jpg"
      },
      %{
        name: "Gödel, Escher, Bach: An Eternal Golden Braid",
        price: 2300,
        author: "Douglas R. Hofstadter",
        category: "Philosophy/Mathematics",
        pages: 777,
        isbn: "9780465026562",
        image_url: "https://covers.openlibrary.org/b/isbn/9780465026562-L.jpg"
      },
      %{
        name: "1984",
        price: 1400,
        author: "George Orwell",
        category: "Dystopian Fiction",
        pages: 328,
        isbn: "9780451524935",
        image_url: "https://covers.openlibrary.org/b/isbn/9780451524935-L.jpg"
      },
      %{
        name: "Animal Farm",
        price: 1200,
        author: "George Orwell",
        category: "Political Satire",
        pages: 112,
        isbn: "9780451526342",
        image_url: "https://covers.openlibrary.org/b/isbn/9780451526342-L.jpg"
      },
      %{
        name: "Treasure Island",
        price: 1100,
        author: "Robert Louis Stevenson",
        category: "Adventure",
        pages: 240,
        isbn: "9780486275598",
        image_url: "https://covers.openlibrary.org/b/isbn/9780486275598-L.jpg"
      },
      %{
        name: "Around the World in Eighty Days",
        price: 1250,
        author: "Jules Verne",
        category: "Adventure",
        pages: 272,
        isbn: "9780486411118",
        image_url: "https://covers.openlibrary.org/b/isbn/9780486411118-L.jpg"
      },
      %{
        name: "A Song of Ice and Fire",
        price: 9000,
        author: "George R.R. Martin",
        category: "Epic Fantasy",
        pages: 4228,
        isbn: "9780553103540",
        image_url: "https://covers.openlibrary.org/b/isbn/9780553103540-L.jpg"
      },
      %{
        name: "The Lord of the Rings",
        price: 2500,
        author: "J.R.R. Tolkien",
        category: "High Fantasy",
        pages: 1216,
        isbn: "9780544003415",
        image_url: "https://covers.openlibrary.org/b/isbn/9780544003415-L.jpg"
      },
      %{
        name: "Batman and Robin",
        price: 2000,
        author: "Various Authors",
        category: "Graphic Novel",
        pages: 144,
        isbn: "9781401237707",
        image_url: "https://covers.openlibrary.org/b/isbn/9781401237707-L.jpg"
      },
      %{
        name: "Maus",
        price: 1900,
        author: "Art Spiegelman",
        category: "Graphic Novel",
        pages: 296,
        isbn: "9780679406419",
        image_url: "https://covers.openlibrary.org/b/isbn/9780679406419-L.jpg"
      },
      %{
        name: "How to Rule the World!",
        price: 2200,
        author: "André de Guillaume",
        category: "Humor/Satire",
        pages: 256,
        isbn: "9780000000000",
        image_url:
          "https://prod-images.tcm.com/Master-Profile-Images/thegreatdictator1940.76858.jpg"
      },
      %{
        name: "Dracula",
        price: 1500,
        author: "Bram Stoker",
        category: "Gothic Horror",
        pages: 418,
        isbn: "9780486411095",
        image_url: "https://covers.openlibrary.org/b/isbn/9780486411095-L.jpg"
      }
    ]
  end
end
